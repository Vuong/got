...
export async function addContactChannelTopic(server, token, channelId, datatype, message, assets ) {
  ...
  if (message == null && (assets == null || assets.length === 0)) {
    var topic = await fetchWithTimeout(`${host}/content/channels/${channelId}/topics?contact=${token}`,
    ...
    var topic = await fetchWithTimeout(`${host}/content/channels/${channelId}/topics?contact=${token}&confirm=true`,
    ...
    var unconfirmed = await fetchWithTimeout(`${host}/content/channels/${channelId}/topics/${slot.id}/subject?contact=${token}`, 
      { method: 'PUT', body: JSON.stringify(subject) });
    checkResponse(unconfirmed);

    var confirmed = await fetchWithTimeout(`${host}/content/channels/${channelId}/topics/${slot.id}/confirmed?contact=${token}`,
